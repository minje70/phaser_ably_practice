...
export class GameScene extends Phaser.Scene {
	...
	private myWall: Phaser.Types.Physics.Arcade.ImageWithStaticBody | null =
		null;
	...
	create() {
		...
		this.socket.on('matchGame', () => {
			...
			if (this.isLeft) {
				...
				this.enemyWall = this.physics.add.staticImage(
					780,
					300,
					'enemyWall'
				);
	...
	update(time: number, delta: number): void {
		...
		if (this.isStart && this.myPaddle) {
			...
			else if (this.upKey1?.isDown)
				this.myPaddle.setVelocityY(-PADDLE_SPEED);
		...
		if (this.myPaddle)
			this.oldPos = { x: this.myPaddle.x, y: this.myPaddle.y };
	}
}
